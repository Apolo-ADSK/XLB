...
class VelocitySet(object):
    ...
    def _init_warp_properties(self):
        ...
        self.c = wp.constant(wp.types.matrix((self.d, self.q), dtype=wp.int32)(self._c))
        self.w = wp.constant(wp.types.vector(self.q, dtype=dtype)(self._w))
        self.opp_indices = wp.constant(wp.types.vector(self.q, dtype=wp.int32)(self._opp_indices))
        self.cc = wp.constant(wp.types.matrix((self.q, self.d * (self.d + 1) // 2), dtype=dtype)(self._cc))
        self.c_float = wp.constant(wp.types.matrix((self.d, self.q), dtype=dtype)(self._c_float))
        self.qi = wp.constant(wp.types.matrix((self.q, self.d * (self.d + 1) // 2), dtype=dtype)(self._qi))
    ...
    def warp_lattice_vec(self, dtype):
        return wp.types.vector(len(self.c), dtype=dtype)

    def warp_u_vec(self, dtype):
        return wp.types.vector(self.d, dtype=dtype)
